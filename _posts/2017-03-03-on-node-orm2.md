...
1. Many annoying niggles, e.g. after closing one database connection is it impossible to create a new connection. The only type supported for rational numbers is `float` rather than `double` (despite the majority of programming languages using double precision floating-point numbers).
